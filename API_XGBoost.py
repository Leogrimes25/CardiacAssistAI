...
try:
    model = pickle.load(open('xgboost_cardiac_model.pkl', 'rb'))
    features = ['oldpeak', 'cp', 'thalach', 'exang']
    print(f"Model Successfully Loaded. Features expect : {features}")
except FileNotFoundError:
    print("Error: File 'naive_bayes_cardiac_model not found .")
    exit()
except Exception as e:
...
def predict():
    try:
        ...
        result_text = "Cardiac Disease" if prediction[0] == 1 else "No cardiac Disease"
